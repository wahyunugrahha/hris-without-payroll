...
class KPIController extends Controller
{
    ...
    private function processKPI(Request $request, $action, $kpi_daily_id = null)
    {
        ...
        try {
            ...
            if ($actionType == 'submitted') $kpiDaily->alasan_reject = null;
            $kpiDaily->save();
    ...
    private function checkRoleAksesKPI($targetUser)
    {
        ...
        if ($currentUser->nik === $targetUser->nik) return 'self';
        
        $bawahan = $this->getBawahan($currentUser);
        if ($bawahan->contains('nik', $targetUser->nik)) return 'atasan';
    ...
    {
        $bawahan = $this->getBawahan($user);
        if ($bawahan->isEmpty()) return collect();
        ...
        if (empty($wajibKpiNik)) return collect();
    ...
    private function getPeriodeData($reqBulan = null, $reqTahun = null)
    {
        ...
        if ($hariIni->day >= 26) $hariIni->addMonth();

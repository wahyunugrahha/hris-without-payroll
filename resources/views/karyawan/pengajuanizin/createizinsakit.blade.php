...
@section('header')
    ...
    <div class="presensi-header">
        ...
        <span class="header-title">Form Pengajuan Izin Sakit</span>
...
@section('content')
    <div class="ios-form-container">
        <form method="POST" action="/pengajuanizin/storeizinsakit" id="frmizinsakit" enctype="multipart/form-data">
            @csrf
            <div class="ios-form-card">
                ...
                <div class="custom-file-upload" id="fileUpload1" style="height: 100px !important">
                    <input type="file" name="sid" id="fileuploadInput" accept="image/*">
                    <label for="fileuploadInput">
                        <span>
                            <strong>
                                <ion-icon name="cloud-upload-outline" role="img" class="md hydrated"
                                    aria-label="cloud upload outline"></ion-icon>
                                <i>Tap to Upload Surat Dokter (SID) </i>
                            </strong>
                        </span>
                    </label>
                </div>
                ...
                <div class="ios-form-group">
                    ...
                    <textarea name="keterangan" id="keterangan" class="ios-textarea validate"
                        placeholder="Detail alasan izin sakit (wajib jika tidak ada SID)" rows="4" required></textarea>
                </div>
        ...
        $(document).ready(function() {
            ...
            function cekPengajuanIzin(tgl_dari, tgl_sampai) {
                $.ajax({
                    ...
                    data: {
                        _token: '{{ csrf_token() }}',
                        tgl_dari: tgl_dari, // Kirim tanggal mulai
                        tgl_sampai: tgl_sampai // Kirim tanggal selesai
                    },
                    cache: false,
            ...
            $("#frmizinsakit").submit(function(e) {
                var dari = $("#dari").val();
                var sampai = $("#sampai").val();
                var jmlhari_val = calculateDays();
                var keterangan = $("#keterangan").val();
                var sid_file = $("#fileuploadInput").val();
                ...
                if (file && file.size > maxSizeInBytes) {
                    Swal.fire({
                        ...
                        text: 'Ukuran file tidak boleh lebih dari 3MB. File Anda: ' + (file.size / (

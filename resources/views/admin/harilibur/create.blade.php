...
@section('content')
    ...
    <div class="page-body">
        <div class="container-xl">
            ...
            <form action="{{ route('harilibur.store') }}" method="POST" autocomplete="off" id="form-libur">
                @csrf
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-transparent border-bottom-1 py-3">
                        <h3 class="card-title mb-0">Form Input</h3>
                    </div>
                    <div class="card-body">
                        {{-- Input Data Utama --}}
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                <label class="form-label small text-muted fw-bold required">Mulai Tanggal</label>
                                <div class="input-icon">
                                    ...
                                    <input type="date" name="tanggal_libur_dari"
                            ...
                            <div class="col-md-3 mb-3">
                                <label class="form-label small text-muted fw-bold required">Sampai Tanggal</label>
                                <div class="input-icon">
                                    <span class="input-icon-addon">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                    ...
                                    <input type="date" name="tanggal_libur_sampai"
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-select @error('jenis_libur') is-invalid @enderror" name="jenis_libur"
                                    ...
                                    <option value="nasional" {{ old('jenis_libur') == 'nasional' ? 'selected' : '' }}>Libur
                                        Nasional</option>
                                    <option value="cuti_bersama"
                                        {{ old('jenis_libur') == 'cuti_bersama' ? 'selected' : '' }}>Cuti Bersama</option>
                                    <option value="lokal" {{ old('jenis_libur') == 'lokal' ? 'selected' : '' }}>Lokal /
                                        Khusus</option>
                                </select>
                                @error('jenis_libur')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="text" name="keterangan"
                                    ...
                                    placeholder="Contoh: Libur Lebaran" value="{{ old('keterangan') }}" required>
                                @error('keterangan')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div id="section_lokasi">
                            ...
                            <div class="alert alert-info bg-blue-lt border-0 mb-3" role="alert">
                                <div class="d-flex">
                                    ...
                                    <div>
                                        <strong>Info:</strong> Jika <strong>tidak ada</strong> Cabang/Departemen yang
                                        dipilih (dibiarkan kosong), maka libur akan berlaku untuk <strong>SEMUA</strong>
                                        (Global).
                                    </div>
                                </div>
                            </div>
                                                    ...
                                                    <div class="col-sm-6">
                                                        <label class="form-check">
                                                            <input type="checkbox"
                                                                ...
                                                                {{ is_array(old('kode_cabang')) && in_array($c->kode_cabang, old('kode_cabang')) ? 'checked' : '' }}>
                                                    ...
                                                    <div class="col-sm-6">
                                                        <label class="form-check">
                                                            <input type="checkbox"
                                                                ...
                                                                {{ is_array(old('kode_dept')) && in_array($d->kode_dept, old('kode_dept')) ? 'checked' : '' }}>
                                                            <span class="form-check-label text-truncate"
                                                                title="{{ $d->nama_dept }}">
                                                                {{ $d->nama_dept }}
                                                            </span>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.check-cabang-item').change(function() {
                ...
                $('#checkAllCabang').prop('checked', allChecked);
                updateCount();
            });

            // Logic "Select All" Departemen
            ...
            $('.check-dept-item').change(function() {
                ...
                $('#checkAllDept').prop('checked', allChecked);
                updateCount();
            });

            function updateCount() {
                $('#count-cabang').text($('.check-cabang-item:checked').length + ' terpilih');
                $('#count-dept').text($('.check-dept-item:checked').length + ' terpilih');
            }
            updateCount();
        });
    </script>
